...
async def test_assign_lead_happy_path():
    async with async_session_maker() as db:  # type: AsyncSession
        ...
        manager = User(
            email=f"manager_assign_{ts}@test.com",
        ...
        agent = User(
            email=f"agent_assign_{ts}@test.com",
        ...
        lead = Lead(
            name="Assign Lead",
            phone="+19990000000",
            email=f"assign.lead_{ts}@test.com",
...
async def test_assign_lead_rejects_inactive_agent():
    async with async_session_maker() as db:  # type: AsyncSession
        ts = int(time.time() * 1000)
        manager = User(
            email=f"manager_inactive_{ts}@test.com",
        ...
        agent = User(
            email=f"agent_inactive_{ts}@test.com",
        ...
        lead = Lead(
            ...
            phone="+19990000001",
            email=f"inactive.lead_{ts}@test.com",
            source=LeadSource.OUTBOUND_CALL.value,
            quality=LeadQuality.COLD.value,
            status=LeadStatus.NEW.value,
        )
...
async def test_bulk_assign_leads_happy_path():
    async with async_session_maker() as db:  # type: AsyncSession
        ts = int(time.time() * 1000)
        manager = User(
            email=f"manager_bulk_{ts}@test.com",
        ...
        agent = User(
            email=f"agent_bulk_{ts}@test.com",
...
async def test_bulk_assign_leads_rejects_missing_leads():
    async with async_session_maker() as db:  # type: AsyncSession
        ts = int(time.time() * 1000)
        manager = User(
            email=f"manager_bulk_missing_{ts}@test.com",
        ...
        agent = User(
            email=f"agent_bulk_missing_{ts}@test.com",
